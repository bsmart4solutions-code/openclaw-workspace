...
| Item | Value |
|------|-------|
| **Public IP** | `104.46.216.98` |
| **Provider FQDN** | `bsmart4solutions-1a16d783-9294-vm.japaneast.cloudapp.azure.com` |
| **User Domain** | `vhaznfrp.gensparkclaw.com` |

- **Provider FQDN** (`bsmart4solutions-1a16d783-9294-vm.japaneast.cloudapp.azure.com`): Reserved for OpenClaw system services. Port 443 serves the gateway, port 8443 serves noVNC. Do NOT modify `/etc/caddy/conf.d/openclaw.caddy` — it is managed by the system and will be overwritten on reconfigure.
- **User Domain** (`vhaznfrp.gensparkclaw.com`): Cloudflare-proxied A record for the user's own services. Traffic goes through Cloudflare's CDN/WAF before reaching the VM. Only Cloudflare-supported ports are accessible (HTTPS: 443, 2053, 2083, 2087, 2096, 8443; HTTP: 80, 8080, 8880, 2052, 2082, 2086, 2095). To deploy a user service, create a Caddy config in `/etc/caddy/conf.d/` (user files are preserved across reconfigures). Example:
  ```
  # /etc/caddy/conf.d/custom.caddy
  vhaznfrp.gensparkclaw.com {
      reverse_proxy 127.0.0.1:3000
  }
  ```
  This serves the user's app on `https://vhaznfrp.gensparkclaw.com` (port 443) via Cloudflare proxy.
...
| **noVNC** | 6080 → 8443 (Caddy) | Web VNC viewer at `https://bsmart4solutions-1a16d783-9294-vm.japaneast.cloudapp.azure.com:8443`. User sees the same browser the agent controls |
...
|-------|-------|
| **Name** | Wong Brandon |
| **Email** | bsmart4solutions@gmail.com |
| **Your Email** | `bsmart4solutions@genspark.email` (this VM's email address — when you see this in the To/Cc of an inbound email, that's you) |
...
A remote desktop (VNC) is **always running** on this VM — the user can access it at `https://bsmart4solutions-1a16d783-9294-vm.japaneast.cloudapp.azure.com:8443` (noVNC). The VNC service provides Xvfb :99, Fluxbox window manager, x11vnc, and noVNC. You do NOT need to start it.
